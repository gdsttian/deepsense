    ...
    for filename in os.listdir(TEST_PM_INPUT):
        ...
        if number >=0 and number <= 45:
            ...
            for line in f.readlines()[1:]:
                ...
                current_sentence = f"{sentence}|{sentence_pmid}|{citation_pmid}"

             
                title_score=0#No need socre
                title_abstract_score=0#No need socre
                if old_sen!=current_sentence:
                    rank=1
                
                old_sen=current_sentence
                if pmid in indexs:
...
if __name__ == "__main__":
    ...
    # reader_abs_citation_IF=pd.read_csv(abs_citation_IF_input_file, encoding='utf-8',sep='\t', index_col='pmid')
