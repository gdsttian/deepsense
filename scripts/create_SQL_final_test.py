    ...
    for filename in os.listdir(TEST_SQL_INPUT):
        ...
        if number >460 and number <= 560:
            ...
            for line in f.readlines()[1:]:
                ...
                ArticleTitle=text_to_wordlist(items[7])
                abstract=text_to_wordlist(items[8])
             
                title_score=float(items[6])
                title_abstract_score=float(items[5])
                title_coverage=float(items[10])
                abstract_coverage=float(items[11])
                rank=int(items[9])
...
if __name__ == "__main__":
    ...
    TEST_SQL_INPUT = '../case_DL_testing_final_sentence_with_all_keywords_score_coverage/'
    
    output = 'case_SQL_testing_sentence_all_keyword_final_complete/'
