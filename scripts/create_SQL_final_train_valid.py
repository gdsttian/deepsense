# -*- coding: utf-8 -*-
'''
Create SQL training/validation data
'''
...
def cal_sum_citation(sentence_year,pmid,citations_dict):
    ...
    if cited_years:
        ...
        return 0
def create_SQL_final_train(path,output,reader_abs_citation_IF,citations_dict,normalized_citation_by_year, num_start,num_end):
    ...
    for filename in os.listdir(path):
        ...
        number = int(filename[len('search_result_cases_search_result_sentence_citation'):-4])
        if number >=num_start and number <= num_end:
            start=time.time()
            print(filename)
            ...
            for line in f.readlines()[1:]:
                ...
                pmid = int(items[2])
                ArticleTitle=text_to_wordlist(items[3])
                abstract=text_to_wordlist(items[4])
                label = int(items[5])
             
                title_score=float(items[6])
                title_abstract_score=float(items[7])
...
if __name__ == "__main__":
    ...
    SQL_INPUT = '../case_sentence_contain_all_keyword_only/'
    ...
    output = BASE_DIR + 'train_final_dataset_sentence_all_keyword_in_abstarct.tsv'
    print("Train:")
    num_start=0
    num_end=400
    create_SQL_final_train(SQL_INPUT,output,reader_abs_citation_IF,citations_dict,normalized_citation_by_year,num_start,num_end)
    
    print("Valid")
    output = BASE_DIR + 'valid_final_dataset_sentence_all_keyword_in_abstract.tsv'
    num_start=401
    num_end=460
    create_SQL_final_train(SQL_INPUT,output,reader_abs_citation_IF,citations_dict,normalized_citation_by_year,num_start,num_end)
