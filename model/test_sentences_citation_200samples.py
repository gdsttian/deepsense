...
from model_utils_trec_covid_bm25 import opcount,plot_top_hist,create_submission,calculate_rank
...
if_train=True 
conti_run=False
TEST_INPUT = '/data/shubo/citation/inputs/file_200sample4rerank/' # Test data folder of pubmed BM search
BASE_DIR = '../input/'
OUT_DIR = '../output/'
MODEL_dir='../model/'
...
for filename in os.listdir(TEST_INPUT):
    ...
    preds = model.predict(data_generater_test_complete(
            TEST_DATA_FILE,
            PRED_BATCH_SIZE,tokenizer,MAX_SENTENCE_LENGTH,MAX_ABSTRACT_LENGTH),max_queue_size=6,
                    steps=TEST_SIZE//PRED_BATCH_SIZE + 1, verbose=2)

    calculate_rank(TEST_DATA_FILE, preds)

    np.save(f"{TEST_INPUT}{TEST_DATA_FILE.split('/')[-1]}.npy", preds)
